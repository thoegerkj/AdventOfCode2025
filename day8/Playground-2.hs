...
connect :: DSU -> [(Double, Int, Int)] -> Int -> (DSU, Int, Int)
connect sets ((_, i1, i2) : edges) 1 =
  if fst (findSet sets i1) == fst (findSet sets i2)
    then connect sets edges 1
    else
      let nsets = union sets i1 i2
       in (nsets, i1, i2)
connect sets ((_, i1, i2) : edges) iterations =
  if fst (findSet sets i1) == fst (findSet sets i2)
    then connect sets edges iterations
    else
      let nsets = union sets i1 i2
...
main = do
  ...
  let points = zip (readPoints $ lines contents) [0 ..]
      ...
      iterations = length points - 1
      (components, le1, le2) = connect pointSets sortedEdges iterations
      ((x1, y1, z1), _) = points !! le1
      ((x2, y2, z2), _) = points !! le2
      solution = x1 * x2
